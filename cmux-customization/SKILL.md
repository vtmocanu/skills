...
## Document Location
This document lives in [github.com/vtmocanu/skills](https://github.com/vtmocanu/skills) at `cmux-customization/SKILL.md`.

> **Note**: This is the source of truth. Generated copies in your agent's skills directory (e.g. Claude Code's `~/.claude/commands/`) are derived from this file via `dot-ai skills generate --repo https://github.com/vtmocanu/skills`. Edit here and regenerate; never edit generated copies. Upstream source: `manaflow-ai/cmux` (skills/cmux-customization/); re-sync from upstream rather than authoring from scratch.
...
4. For app settings and cmux-owned shortcuts, use the settings helper from the sibling `cmux-settings` skill. Its install location depends on the host: `~/.claude/commands/dot-ai-cmux-settings/scripts/cmux-settings` under dot-ai/Claude Code, `~/.agents/skills/cmux-settings/scripts/cmux-settings` under the upstream cmux installer, `~/.codex/skills/cmux-settings/scripts/cmux-settings` under `skills.sh`. Discover whichever is present:
   ...
   "$CMUX_SETTINGS_HELPER" list-supported
   "$CMUX_SETTINGS_HELPER" set browser.openTerminalLinksInCmuxBrowser true
