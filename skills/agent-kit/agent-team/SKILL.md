...
- **TUI surface**: a terminal UI the team ships. Detect the framework in the manifests — Go `bubbletea`/`lipgloss`/`tview`/`tcell` in `go.mod`, Rust `ratatui`/`cursive` in `Cargo.toml`, Python `textual`/`urwid` in `pyproject.toml`, Node `blessed`/`ink` in `package.json` — plus `**/tui_*.go` / `**/*_tui.go` / `**/tui/**` paths and any `*.tape` (vhs). A hit means the repo has a TUI to review; record HOW it renders for offline review (an existing screenshot/snapshot harness, or none — then propose one), because the `tui-ux` role's tail needs a concrete render command or the role has no instrument.
...
- **spec-keeper**: include if a `specs/` directory exists OR the user asks for spec tracking. Maintains `specs/human.md` (user-stated requirements; edits gated on user confirmation) and records AI design decisions in ADRs and PRD Decision Logs (auto-applied; a legacy `specs/ai.md` is kept until it is frozen), aiming for rebuild-from-specs sufficiency.
...
- **tui-ux**: include if any of the role's `triggers_on` patterns match (a terminal UI surface: `tui_*.go`/`*_tui.go`/`tui/`, a `*.tape`, or a TUI framework named in a manifest — see the TUI-surface signal in Step 1). Validates TUI work by rendering it to light/dark images offline (charmbracelet/freeze, or the framework's own snapshot testing) and READING them, and reviews status legibility, NO_COLOR fallback, width/layout, terminal-injection safety, and navigation; read-only — the terminal analogue of web-ux. Its `## For this repo` tail MUST name the concrete render command (the repo's screenshot harness) — without it the role has no instrument; if none exists, propose building one. Dispatch it whenever the team's change touches the TUI.
...
5. On confirmation, apply targeted edits:
   - **Version bump**: `apply <role> [<role> ...]` replaces the generic body (everything ABOVE the `## For this repo` heading) with the current `roles.yaml` body, rewrites the frontmatter `version:` and `description:` (the description is library-owned like the body: `check` reports a differing one as MODIFIED, so a library rewording has to land or it reads as drift forever), and leaves the tail, `tools:` and `model:` untouched. This is exactly why Step 3 keeps repo-specifics in the tail — the generic part is replaceable wholesale. The script re-reads each file after writing and aborts if the tail moved, comparing BYTES: "the tail was preserved" is the one claim worth checking against the file rather than against the string it just built, and a check that compares two newline-normalized strings cannot see a CRLF file being silently rewritten.
   - **A body or description that differs at EQUAL version** is the axis-3 case, not staleness, and `apply` REFUSES it: the difference is unexplained, so overwriting it destroys a local change nobody decided to discard. Read it, give it the backport verdict this step already requires, then re-run with `--force` if the verdict is `keep library`. This is the only content guard, and it is the one that fires on the signal that actually means "a human edited this".
...
- **Scale the wave to the diff's RISK CLASS; do not fix it.** A trust-boundary, data-integrity, auth, or untrusted-input diff gets the full read-only wave (reviewer + auditor + tester, the tester running mutation controls); a pure-presentation, copy, refactor, or config diff gets a SINGLE reviewer for one clean round. The classifier is the DIFF, not the milestone's convenience — when unsure, treat it as trust-boundary. Evidence (2026-08-15): a reviewer+tester pair on a run's one trust-boundary milestone caught a real control-byte injection via mutation testing, while single-reviewer rounds on three presentation-only milestones cost nothing and lost nothing. The product `lead.md` already scales depth this way and does not name a fixed reviewer-then-auditor pair; this dev-team flow should match it. (This is the cut item 8 proposes from; the depth here is the assurance it must not cut on a risky diff.)
- **A trivial, fully-specified, mechanical edit is the LEAD's to make inline, not a coder's to be dispatched.** Spinning up a strong-model coder to apply a one-line rename, a copy tweak, a version bump, or a fix you have already fully specified pays a cold-start plus a re-read of files you already hold, for an edit that carries no design decision. Make it yourself, commit it by explicit path, and send it through the same risk-scaled review as any other change (a presentation/refactor diff is one reviewer). Reserve a coder dispatch for work that needs implementation judgment or a file scope you are not already holding. (A large cluster of judge cost-efficiency recommendations reduces to this and to the wave-depth bullet above: do not route a trivial diff to a strong model or a full validator wave.)
- **Pin review scope to explicit commit SHAs** in the review dispatch, and if the worker adds commits after the review was dispatched (a follow-up fix, a crossed-in-flight reconciliation), immediately SendMessage the reviewer the new tip and require explicit confirmation that ALL commits were covered. Observed failure mode: a reviewer's report cited only the first of two commits on the branch; the second commit was verified only after a direct "did you cover SHA X?" follow-up. A second tell (observed 2026-07-05): a validator's report describing behavior that CONTRADICTS the worker's description of a later commit (e.g. praising semantics the follow-up reversed) means the validator reviewed a stale tip — require a re-read at the live tip and an explicit ruling before accepting either claim. **The lead-side half of the same protocol: require every report to LEAD with its tip SHA, and verify THAT SHA, never `HEAD`** — `HEAD` moves under you while the worker works, so a `HEAD`-based probe is a race you lose silently (validated 2026-07-21, proposed by the coder after it cost a round-trip: the lead grepped for a symbol at `HEAD`, found nothing, and declared seven items unlanded; they had landed one commit later). **And distinguish EARLY from STALE when a probe disagrees with a report** — they have different fixes and the familiar one hides the other. Stale = you read a value that has since changed (fix: pin what you verify). Early = you ran the probe before the worker applied the change, so your result was correct when taken and correct forever *at that SHA* (fix: re-probe, do not re-dispatch). Same run, both occurred; the second was nearly mis-attributed to a bad grep pattern, which would have "fixed" a grep that was never wrong. **Make this a precondition, not a caution: before sending ANY message a probe triggered — a re-dispatch, a GO re-send, a "did not land" nudge — re-run the probe pinned to the report's STATED tip SHA (never `HEAD`, never the worktree's current checkout, which may sit at a commit the worker has already moved past), and NAME that SHA in the message. A probe that agreed at its own SHA is EARLY, not stale — re-probe at the reported tip, do not re-send. Which SHA you probed is either in the message or it is not, so this gate is checkable while composing.** (Measured 2026-08-08: a lead read a worktree HEAD one commit before a spec-keeper's write landed, mis-read the disagreement as a message crossing, and re-dispatched a redundant GO; the worker re-verified live HEAD and correctly no-op'd it. The re-probe-at-the-reported-tip gate above would have caught it.)
  ...
  **The lead is a live writer too.** Every follow-up commit you land while a validator is reading the tree is the writer the paste above exists to detect, and the paste you took before dispatch does not cover it. Measured 2026-09-03 (a downstream project): all coders had protocol-confirmed shutdown, the dispatch said "no writer is live", and the lead then landed four small follow-up commits in that worktree while the reviewer read it; the reviewer quoted a clause that only existed two commits after the pin. Either hold follow-up commits until the report lands, or tell the validator up front to read every path by `git show <sha>:<path>` and name each new tip as it lands, one message per tip.
...
- **A backgrounded gate's log goes WORKTREE-LOCAL, and its exit code is captured in the SAME run.** Writing a background gate's stdout to `/tmp` (or anywhere outside the worktree) and then reaching for `Read`/`sleep`+`tail` to poll it is a dead end when the sandbox confines file access to the worktree — both are blocked, and the recovery costs round-trips. Redirect to a gitignored path inside the worktree and capture the status in one run (`... > <worktree>/gate.log 2>&1; echo "EXIT=$?" >> <worktree>/gate.log`) rather than re-running the whole suite to learn whether it passed. Several judge cost-efficiency recommendations reduce to exactly this: capture the gate's exit code and output once; do not spend a second full run reading a result the first run already produced.
...
- If the spec-keeper role exists: once blocking findings are resolved, dispatch it with the change summary plus the user-vs-AI provenance breakdown (see Step 3). It applies decision-record updates (ADRs, PRD Decision Logs) directly and sends proposed `specs/human.md` edits back; relay those to the user for confirmation before telling the spec-keeper to apply them. The task is not complete until specs are in sync — the bar is that the code could be rebuilt from `specs/human.md` plus the decision record alone, with `specs/human.md` treated as the binding contract.
...
- **VERIFY every finalize INVARIANT against the actual branch diff, not against the plan that promised it.** A postcondition the plan asserted — "no `.github/workflows/` edits", "migrations renumbered", "no new dependency" — is a claim about the branch, and a clean working tree plus a list of commit subjects does not establish it. Run the diff that would falsify it and read the result (`git diff --name-only <base>...HEAD -- .github/workflows/` must be empty for the workflow-scope invariant). Measured: a lead's finalize step confirmed a clean tree and commit subjects and declared the no-workflow-edits invariant satisfied without ever diffing the branch for workflow-file changes.
...
8. **Cost efficiency — quality-preserving reductions only.** Item 7 measures; this item judges what the measurement licenses. Using item 7's wall-clock / concurrency / idle figures plus the roster actually dispatched, name where the run spent tokens or wall-clock WITHOUT buying assurance, and propose cuts — **quality first: never propose a reduction that removes an assurance that caught, or plausibly could catch, a real defect.** The test for each candidate is "would this run still have caught its worst finding?" — if no, it is not a candidate.

   - **Separate the catch from the cut.** First name the run's highest-value verification event (the finding that would have shipped without it), and mark the assurance that produced it PROTECTED — never a cut candidate, however expensive. (Worked example: a mutation-testing review wave on a trust-boundary diff surfaced an unsanitized-untrusted-field control-byte injection; that wave is protected.)
   - **Then propose from the residue,** each lever only where it removes REDUNDANCY, not scrutiny: risk-proportional wave depth (single reviewer on a presentation/refactor diff; full reviewer+tester(+auditor) reserved for trust-boundary / data-integrity / auth / untrusted-input diffs — Step 4); pasting the pointers and tree-evidence the lead already holds instead of buying N cold re-derivations; pointing corrections at the brief instead of re-pasting them; trimming a pasted teammate report to its load-bearing summary rather than dropping it; and cutting idle round-trips by pipelining the next unit under a running validator wave.
   - **State the trade you are NOT making.** A cost item that cannot name a single PROTECTED assurance has not analysed — it has only found things to remove. Report the protected set first, the residue second.
