...
| [session-peers](skills/session-peers/SKILL.md) | Make Claude Code sessions and Codex CLI threads on one machine message each other: Codex threads can auto-attach as real peers, aliases follow renames, stale bridge metadata is collected safely, and replies return through Claude's inbox socket. Auto-attachment requires running `~/.agents/skills/session-peers/scripts/peers.py install-hook --auto-attach` once, then trusting the entry through Codex `/hooks`; see [setup](skills/session-peers/SKILL.md#automatic-attachment-recommended). |
