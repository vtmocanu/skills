...
- **Codex auto-update:** add an equivalent `SessionStart` handler to `~/.codex/hooks.json` with matcher `startup|resume`, then review and trust it with `/hooks`. Claude and Codex share the `.agents` lockfile/store, so installations using both hooks must call one shared lock-taking wrapper rather than duplicate the raw npx chain. The [skill-maker hook section](skills/skill-maker/SKILL.md#auto-install-new--refresh-on-session-start-hook) has the exact contract.
