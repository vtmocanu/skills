...
| [session-peers](skills/session-peers/SKILL.md) | Make Claude Code sessions and Codex CLI threads on one machine message each other: Codex threads can auto-attach as real peers, aliases follow renames, stale bridge metadata is collected safely, and replies return through Claude's inbox socket. |
