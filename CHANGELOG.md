...
Workflow-only. **No role body changed and no role `version:` moved**, so a consumer
regenerating from this release will NOT see its roster reported stale.

### Added

- `agent-team`: Mode 1 Step 1 now **measures the root `CLAUDE.md` and reports the number**, because a project `CLAUDE.md` is loaded in full at the start of every session and a subagent is its own session: an N-token file costs N tokens *per teammate spawned*, not N per task, and nothing amortises it (each subagent is a separate conversation, so there is no cross-agent prompt-cache hit). Past roughly 30KB the file is a standing tax on a team that fans out eight agents in a wave, against Claude Code's own under-200-lines guidance. Where it exceeds that, the split is proposed as one more numbered item in the Step 4 proposal: the root keeps what is repo-wide, per-component detail moves to `.claude/rules/*.md` with `paths:` frontmatter, which loads only when a matching file is read. Measured 2026-08-04 on a four-toolchain repo, 578 lines / 124,514 bytes became 188 lines / 40,507 bytes, so an eight-agent wave stopped paying about 170k tokens of preamble it could not use. The step also requires the move be proven byte-exact by reassembly diff, since a split is a fidelity problem before it is a formatting one, and it records the two mechanism properties that fail silently: a path-scoped rule fires on a file READ, so running a gate without opening a file in that component does not pull its rule in, and nested rules are not re-injected after `/compact` while the root file is.
- `agent-team`: Step 1 discovery gains a **dead weight and too-large-to-read** probe. Directories imported by nothing (archived evidence, vendored corpora, completed-work archives) get recorded so sweeps skip them, and single files big enough to end an agent get recorded with their size and a scoped alternative. A 1.3MB spec file is roughly 337k tokens, so one `Read` costs a teammate, and the instinct that triggers it is the careful one: reading the spec before starting.
- `agent-team`: Mode 3 Step 4 gains **paste the pointers you already found**. A dispatch saying "go find X" buys N re-derivations of a search the lead has already run, because validators cold-start with no memory of the investigation, so a reviewer, auditor, fact-checker and tester sent at one diff each independently grep for the same symbols. On a large repo that sweep is the largest slice of a teammate's context before it does any work. This is not *Context handoff* restated: that rule is about the prompt being self-contained, this one is about duplication. It licenses pasting the LOCATION only, never the lead's conclusion about what is there (which would author the control for its own claim), and a pointer carries a citation's obligation to be pinned to the dispatched SHA. Where a sweep is genuinely needed and its product is a conclusion rather than a corpus, it goes to a read-only search subagent whose context is discarded so the caller pays a paragraph.

### Changed

- `agent-team`: the **>30% recycle threshold is now weighed against the prompt cache** rather than applied mechanically. A recycle discards a warm cache and makes the respawned teammate re-read its role file, the project `CLAUDE.md` and its checkpoint at full price, then re-derive whatever the checkpoint missed. Where the session cache TTL is long (an hour rather than the five-minute default), a coder continuing down the same lane across several milestones can be both cheaper and faster kept resident, even above 30%. The threshold is a heuristic about RELEVANCE rather than size, so it applies when scope genuinely changes, which is what "NEW task" already said. The >85% force-recycle is explicitly exempted from the caveat.
