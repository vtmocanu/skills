...
- `agent-team`: spec-keeper (v4 to v6): the AI-decision record is no longer a `specs/ai.md`. Decisions go to an ADR (created under `adr/` when the repo has none) when they outlive the work, and to the originating PRD's or design doc's Decision Log otherwise; the `## For this repo` tail may name the record; the role never creates a `specs/ai.md`. Legacy: a repo that already has one keeps recording there until the file opens with a `FROZEN` marker, which the role honors on the `ai.md` read its workflow already performs (a tail-free product subagent never sees the repo's CLAUDE.md, so the marker is the channel that reaches it). SKILL.md's spec-keeper roster line and dispatch guidance updated to match. Versions bumped so consumers resync. Regenerated `product-agents/spec-keeper.md`. Motivated by vtmocanu/uzi#1317, where `ai.md` duplicated the PRD Decision Logs, was append-only in practice, and collided on every close landing.
...
### Changed
- `agent-team`: two role-body fixes surfaced by CodeRabbit on the downstream uzi sync PR (vtmocanu/uzi#1092), versions bumped so consumers resync. fact-checker (v8 to v9): the "reintroduce the defect" fold runs in a detached throwaway worktree, never in the shared tree, and the original worktree is shown untouched afterwards (the reviewer and auditor bodies already said so). architect (v7 to v8): the write surface is the design-doc directory the `## For this repo` tail names, with `docs/adr/` and `docs/design/` as common examples rather than a fixed list. Regenerated `product-agents/{architect,fact-checker}.md`. SKILL.md Mode 3 Step 4 gains "the lead is a live writer too": hold follow-up commits while a validator reads the tree, or have it read by `git show <sha>:<path>` and name each new tip. `sync.py apply` now also rewrites the `description:` line (library-owned like the body; the same equal-version refusal and `--force` apply), because a library description change could never be synced before and read as MODIFIED drift forever; three tests added.
- `agent-team`: terse generic role bodies. All 14 `prompt_body` blocks in `roles.yaml` were rewritten to keep every rule and drop the discovery stories, dates, retractions and self-reference (98.8 KB → 70.2 KB; per-role −9% to −49%; `skill-reviewer` untouched, it was already terse). Each body was verified by an independent rule inventory of the original against the rewrite (no rule missing, weakened or changed). Versions bumped for the 13 changed roles so downstream consumers resync: coder 12, reviewer 14, auditor 11, tester 12, documenter 5, release 5, spec-keeper 4, fact-checker 8, web-ux 9, tui-ux 2, ux-designer 5, architect 7, researcher 5. Two new generic rules ride the same bump: coder and tester run a gate once to a log and return its status (measured four duplicate gate runs in one uzi run); web-ux treats a browser-CLI launch failure as an environment finding with a three-attempt budget (uzi #1082). `researcher` moves to the sonnet tier (read-only investigation is mechanical; uzi #1080).
...
- `agent-team`: a CodeRabbit-driven reviewer role-body fix (version bumped so downstream consumers resync). reviewer (v12 to v13): deleting the last literal caller of a helper makes it a dead-code candidate, not proof it is dead, since reflection, DI, or a config-driven path can still reach it; the cascading case is held to the same reachability checks the sentence above already requires. Regenerated `product-agents/reviewer.md`. Surfaced on downstream uzi PR #971 (net reviewer range this cycle: v11 to v13).
...
- `agent-team`: three CodeRabbit-driven generic role-body improvements (versions bumped so downstream consumers resync). auditor (v9 to v10): a missing size or item cap on an external-controlled read is now a finding independently of any wall-clock timeout, since a timeout does not bound body size. reviewer (v10 to v11): "no references" from `git grep` makes a symbol a dead-code CANDIDATE, not proof, so check dynamic dispatch, reflection, DI/plugin registration, generated code, and config-driven entry points before calling it dead. web-ux (v7 to v8) and ux-designer (v3 to v4): guard the `agent-browser session id` capture and abort on an empty value, so a failed session id does not silently fall back to the shared browser tab.
