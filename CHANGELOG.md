...
- `agent-team`: **`scripts/sync.py`, the only executable this skill currently ships.** The load-time staleness pass is prescribed as mandatory and compares the generic BODY as well as the `version:`, because `roles.yaml` allows one bump per release rather than one per edit, so a body change can ship without an increment and is invisible to a version-keyed comparison by construction. That check had no tooling, so every session either re-derived an 11-file body comparison by hand or wrote a throwaway to do it. `check` classifies each file as `ok` / `STALE` / `MODIFIED` (equal version, content differs) / `CUSTOM` / `LEGACY` / `BAD-FM`, and `apply` performs the Mode 2 Step 5 merge: generic body replaced from the library, `version:` rewritten, `## For this repo` tail preserved byte-exact — verified by re-reading the file and comparing BYTES, which is what makes the claim survive a CRLF input. Exit codes separate a finding (1) from the instrument failing (2), so an unreadable library cannot be read as drift that was never measured.
- `agent-team`: **`apply` refuses rather than overwriting** a role with no library entry, a role whose name would resolve outside the agents directory, a symlinked agent file, a frontmatter a strict parser rejects, a body that differs at EQUAL version (the axis-3 local modification; `--force`), and and a body carrying lines the library lacks is DROPPED with a warning rather than refused (see below). The last is about the body and not about whether the file has a tail: a file whose body is purely a stale copy has nothing to preserve and syncs normally, tail or no tail. Every guard runs over every named role before the first byte is written, so exit 1 always means the tree is untouched.
...
- **Exit-code contract violations:** an empty agents directory, a malformed library entry, `roles:` as a mapping, and a non-UTF-8 agent file all exited 1 (drift) rather than 2 (instrument failure). `apply` could also return 1 — documented as "nothing applied" — after writing files; the version stamp now runs in the guard phase, so 1 always means the tree is untouched.
- **Round 2 of review found the fix for the inline-tuning guard had over-corrected into a worse defect, and it is now a WARNING rather than a refusal.** Counting a `replace` opcode toward "lines the library lacks" made every library REWORDING look like repo-owned content; counting `insert` only fixed that half, but a line the library DELETED is still present in an older copy and is indistinguishable from one a human added. Measured: refusing fired on 17 of 51 real library edits in this repo's own history, on **11 of 11 roles** for the bump that fixed an unreachable report recipient — the release this skill cites as motivating the body diff — and on 5 of 11 roles of a roster generated from an older library revision. The escape hatch was a flag whose own help said it destroyed content, so the guard trained the operator to force past it. `apply` now drops such lines, names the count on stderr, and its closing "the only deletions should be the old version lines" line tells the truth instead. `--force-inline` is retired; `--force` and the equal-version refusal remain, which is the case where the difference is genuinely unexplained.
