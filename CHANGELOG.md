...
Covers one commit that had been sitting unreleased on `main` since 0.19.0 (the 9-role reflect pass) plus a new reflect pass from a 6-milestone, 11-role run. Most role bodies moved, so a consumer regenerating from this release should expect `/agent-team update` to report its roster stale. Net `version:` deltas from 0.19.0: coder 5 to 6, reviewer 5 to 7, auditor 5 to 7, tester 6 to 8, documenter 3 to 4, fact-checker 5 to 6, spec-keeper 2 to 3.
...
### Previously unreleased (landed on `main` after 0.19.0)
