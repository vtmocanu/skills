...
- **No live readout exists.** Stated as a conclusion, not a citation: the telemetry Claude Code exports is an asynchronous per-request export, and nothing in the documented tooling offers a live query for a running subagent's current context. **The docs contain no sentence saying this** — I checked, after quoting one that did not exist (see the note at the end of this list).
...
**Force-recycle regardless of coupling** when a teammate has **compacted at least once** (grep its transcript for `compact_boundary`), or when it reports its own degradation, or when you can no longer reconstruct what it is carrying. Even on a same-track continuation, externalize via checkpoint and respawn. Note this is weaker than the `>85%` figure it replaces on TWO axes, and the second matters more. It is weaker in rationale, because auto-compaction means the session is not at risk of dying, so what you protect is the quality of a teammate now reasoning over a summarised history. And it is weaker in TIMING, which is the half that changes how you use it: on one host's transcripts, four subagent auto-compactions carried `preTokens` of 975k-1002k against a 1M window, i.e. this trigger fired near 98% where `>85%` fired at 85%. Treat that as one small sample rather than a constant (a fifth event on the same host, a MANUAL compaction of a main conversation, sat at 354k), but the SHAPE does not depend on the sample: the trigger is necessarily post-hoc, since it can only be true after the summarisation you wanted to avoid. It is the honest trade for an instrument that exists over one that did not.
