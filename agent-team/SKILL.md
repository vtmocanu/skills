...
5. On confirmation, apply targeted edits:
   ...
   - **So read the warning, and read `git diff`.** `check` marks such a file `LEGACY` and says how many lines go; `sync.py diff <role>` lists them. If any is genuinely repo-specific it belongs in the `## For this repo` tail — move it there BEFORE syncing. The tail itself is never at risk; this is only ever about content someone put in the library-owned half.
