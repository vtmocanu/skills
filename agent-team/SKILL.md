...
It exits **0** when clean, **1** when anything drifted, **3** from `apply` when it dropped body lines the library lacked (each such file gets a `.pre-sync` backup beside it), **2** when the instrument itself failed — an unreadable library is not a finding about the repo, and a caller that reads it as one goes hunting for drift that was never measured. `check` and `diff` write nothing.
