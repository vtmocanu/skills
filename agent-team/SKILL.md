...
**For anything beyond a small fix, dispatch a DESIGN-CRITIQUE wave BEFORE the implementer, and treat the design as FROZEN once the implementer spawns.** Send reviewer + auditor (and architect, if the roster has one) the brief ALONE — no code exists yet — and ask them to attack the design: is the mechanism it asserts actually the mechanism in the code? what does the proposed test structurally fail to catch? Settle what comes back, rewrite the brief, and only then spawn the coder, once, against a frozen spec.
...
- **THE BRIEF IS THE SPEC. Corrections AMEND THE BRIEF; messages only point at it.** When a requirement changes, edit `.claude/agent-team-tasks/<slug>.md` and send a short message naming the section that moved — never a long message carrying the requirement itself. Date each amendment inside the brief so a worker can tell what it has already seen.
  ...
  **Check the brief is not gitignored before relying on it.** Some repos ignore `.claude/agent-team-tasks/` (uzi does, `.gitignore:44`), which makes the spec this rule leans on untracked: invisible to reviewers, absent from the branch, and unavailable to a later reflect pass reconstructing what was actually asked. It also quietly defeats Step 1's "put the agent-team artifacts on the work branch". If it is ignored, either raise un-ignoring it with the user or keep the brief somewhere tracked — and say which you did.
