...
- Run a gate once, to a log inside the worktree, then read the log: `<gate command> > "gate-$$.log" 2>&1; rc=$?; echo "EXIT=$rc" >> "gate-$$.log"; test "$rc" -eq 0`. Name the log per run (`$$`) and keep it on a path the repo ignores (add the pattern if it is not), so a shared worktree never shows another agent your artifact and it can never be staged; a sandbox may confine reads to the worktree, which is why it stays inside it. Never rerun the same gate on the same tree to read its output differently; a second run is the same measurement paid twice, and under contention a flakier one.
